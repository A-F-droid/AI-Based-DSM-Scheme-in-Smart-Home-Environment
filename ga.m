...
Nvar = size(FieldDR,2); % 2 tells how many columns are. 1 for rows 
aux = rand(Nind,Nvar);
m=[-1 1]*FieldDR;
ublb=ones(Nind,1)*m;
...
for i=1:nind
    if isempty(gaDat.ObjfunPar)
        [ObjV(i), x]=feval(gaDat.Objfun,Chrom(i,:));
    else
        [ObjV(i), x]=feval(gaDat.Objfun,Chrom(i,:),gaDat.ObjfunPar);
    end
    Chrom(i,:)=x;
end
gaDat.ObjV=ObjV;
...
% gaiteration(gaDat) TO SHOW RESULTS OF EACH ITERATION
